...
import { Clock, Globe, Mail, MessageCircle, ArrowDown } from "lucide-react";
      ...
      <PageHero
        ...
        cta={
          <button 
            ...
            className="flex items-center gap-2 text-white/80 hover:text-gold transition-colors uppercase tracking-[0.2em] text-xs font-bold"
          >
            Jump to form <ArrowDown size={14} />
          </button>
        }
      />

      <SectionWrapper bg="secondary">
        <div id="contact-form" className="grid lg:grid-cols-2 gap-20">
          {/* Direct channels */}
          <div className="space-y-12">
            ...
            <div className="flex gap-6">
              ...
              >
                <div className="p-2 border border-white/10 rounded-lg group-hover:border-gold/40">
                  <Globe size={20} />
                </div>
...
function ContactMethod({
  ...
  return (
    <Reveal>
      ...
      >
        <div className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center group-hover:border-gold/40 transition-colors shrink-0">
          {icon}
        </div>
        <div>
          <h4 className="text-white font-medium mb-0.5">{title}</h4>
          <p className="text-gold text-sm mb-1">{detail}</p>
